...
template = pd.read_excel("D:/1. Work/1. Daily/Production follow up/template - Copy.xlsx")
...
for index, row in template.iterrows():
    ...
    if unit in units:
        ...
        template.loc[index, 'QC Pass (Pcs)'] = sum(QC_Pass[:-2])
        template.loc[index, 'Accu. QC Pass'] = sum(Accu_QC_Pass[:-2])
        template.loc[index, 'Plan Balance'] = sum(plan_balance[:-2])
        template.loc[index, 'Required Prod upto yesterday'] = sum(req_prod_upto_yes[:-2])
        template.loc[index, 'Back Log upto yesterday'] = sum(backlog_upto_yes[:-2])
        template.loc[index, 'QC Pass in minutes'] = sum(QC_pass_min[:-2])
        template.loc[index, 'Factory Target'] = sum(target[:-2])
        template.loc[index, 'Plan Achv % (monthly, minute based)'] = str(setPrecision(sum(plan_achv[:-2]) / (len(plan_achv) - 2), 2)) + '%'
        avg_prod_smv = setPrecision(sum(prod_smv[:-2]) / (len(prod_smv) - 2), 2)
        avg_plan_smv = setPrecision(sum(plan_smv[:-2]) / (len(plan_smv) - 2), 2)
        template.loc[index, 'Produced SMV'] = setPrecision(sum(prod_smv[:-2]) / (len(prod_smv) - 2), 2)
        template.loc[index, 'Produced Hr'] = setPrecision(sum(prod_hr[:-2]) / (len(prod_hr) - 2), 2)
        avg_prod_eff = sum(prod_eff[:-2]) / (len(prod_eff) - 2)
        avg_prod_eff = setPrecision(avg_prod_eff, 2)
        template.loc[index, 'Prod. Eff%'] = str(avg_prod_eff) + "%"
        total_acc_prod_min = avg_prod_smv * sum(Accu_QC_Pass[:-2])
        template.loc[index, 'Accu. Produced Minute'] = int(total_acc_prod_min)
        total_total_plan_min = avg_plan_smv * sum(req_prod_upto_yes[:-2])
        ...
        template.loc[index, 'Average Requirement as per Plan'] = sum(avg_req_per_plan[:-2])
        template.loc[index, 'Average target in minutes'] = sum(avg_trg_min[:-2])
        template.loc[index, 'Required Hr'] = int(sum(req_hr[:-2]) / (len(req_hr) - 2))
        avg_plan_eff = setPrecision(sum(plan_eff[:-2]) / (len(plan_eff) - 2), 2)
        template.loc[index, 'Plan Eff%'] = str(avg_plan_eff) + '%'

est_val = sum(QC_Pass[:-2])
